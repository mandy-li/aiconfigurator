...
This guide explains how to set up and run oneCCL collective communication benchmarks on Intel XPU (GPU) devices using `collect_oneccl.py` or `collect_comm.sh --device xpu`.
...
### Option B: Run Individual Operations via `collect_oneccl.py`

```bash
cd collector/

# all_gather with 2 GPUs, half precision, default range (512B to 512MB)
python collect_oneccl.py --oneccl_op all_gather --dtype half --num_gpus 2

# reduce_scatter with 2 GPUs
python collect_oneccl.py --oneccl_op reduce_scatter --dtype half --num_gpus 2

# all_reduce with 4 GPUs, custom range
python collect_oneccl.py --oneccl_op all_reduce --dtype half --num_gpus 4 --range "1024,268435456,2"

# alltoall with int8
python collect_oneccl.py --oneccl_op alltoall --dtype int8 --num_gpus 2
```

### CLI Options for `collect_oneccl.py`
